...
export const getRouteCords = async (cargo: ICargo): Promise<IRouteCords> => {
  console.log(cargo.from, cargo.destination);
  if (cargo.from && cargo.destination === undefined) return DEFAULT_ROUTE_CORDS;
